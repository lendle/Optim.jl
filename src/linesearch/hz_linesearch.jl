...
#   - The Wolfe conditions are checked only after alpha is
...
#   - In step I2, we multiply by psi2 only if the convexity
...
function alphatry{T}(alpha::T,
    ...
    alphamax = min(alphamax, tocorner(x, s, constraints))
    alphatest = min(alphatest, alphamax)

    phitest = d.f(step!(xtmp, x, s, alphatest, constraints))
    f_calls += 1
...
function hz_linesearch!{T}(df::Union(DifferentiableFunction,
    ...
    while !isbracketed && iter < linesearchmax
        ...
        elseif lsr.value[end] > philim
            ...
            ia, ib, f_up, g_up = bisect!(df, x, s, xtmp, g, lsr, ia, ib, philim, constraints, detailed_trace)
            f_calls += f_up
            g_calls += g_up
            isbracketed = true
        else
    ...
    while iter < linesearchmax
        ...
        iswolfe, iA, iB, f_up, g_up = secant2!(df, x, s, xtmp, g, lsr, ia, ib, philim, constraints, delta, sigma, detailed_trace)
        f_calls += f_up
        g_calls += g_up
        if iswolfe
        ...
        if B - A < gamma * (b - a)
            ...
            ia, ib, f_up, g_up = update!(df, x, s, xtmp, g, lsr, iA, iB, length(lsr), philim, constraints, detailed_trace)
            f_calls += f_up
            g_calls += g_up
        end
        iter += 1
...
function secant2!{T}(df::Union(DifferentiableFunction,
    ...
    iA, iB, f_up, g_up = update!(df, x, s, xtmp, g, lsr, ia, ib, ic, philim, constraints, detailed_trace)
    f_calls += f_up
    g_calls += g_up
    if detailed_trace & SECANT2 > 0
...
function update!(df::Union(DifferentiableFunction,
    ...
    return bisect!(df, x, s, xtmp, g, lsr, ia, ic, philim, constraints, detailed_trace)
end
...
function linefunc!{T}(df::Union(DifferentiableFunction,
                                TwiceDifferentiableFunction),
                      x::Array{T},
                      s::Array,
                      alpha::Real,
